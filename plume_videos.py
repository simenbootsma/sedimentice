...
def main():
    ...
    # compute_breadth_heatmap(sz, debug=True)
    # show_breadth(sz)
        ...
        for trk in tracks:
            ...
            if len(trk) > 0:
                ...
                if len(trk) > 1:
                    ...
                    p0 = trk[:, :2] + vec_orth
                    p1 = trk[:, :2] - vec_orth
                    pts = [np.vstack((p0, np.flipud(p1), p0[0, :])).astype(np.int32)]
                    cv.fillPoly(trk_map, pts, (1, 1, 1))

                    # plt.figure()
                    # plt.imshow(trk_map, cmap=cmo.haline)
...
def breadth_profiles():
    ...
    for sz in sizes:
        ...
        print("[{:s} left]: abs std = {:.1f} mm  |  rel std = {:.1f}%".format(sz, np.nanmean(std_l),
                                                                              np.nanmean(std_l / avg_l) * 100))
        print("[{:s} right]: abs std = {:.1f} mm  |  rel std = {:.1f}%".format(sz, np.nanmean(std_r),
                                                                               np.nanmean(std_r / avg_r) * 100))
...
def generate_particle_cloud(sz, debug=False):
    ...
    for n in range(n_frames):
        ...
        circles = find_particle_centers(gray, min_radius, max_radius)
        for c in circles:
            cloud.append(c + [n])
...
def analyse_tracks(sz):
    ...
    ws_avg = np.abs(settling_velocity_above_z(dp, 293.15, 0, 2500, -0.1))  # average settling velocity over height H = 0.1 m
...
def find_particle_centers(img, min_radius, max_radius):
...
def parabolic_peak_fit(x, y):
    ...
    """
    # TODO: change to Gaussian peak fit? (see PIV slides)
    x1, x2, x3 = x
    y1, y2, y3 = y
